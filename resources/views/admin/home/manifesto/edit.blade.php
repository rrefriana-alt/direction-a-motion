...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Manifesto — {{ strtoupper($locale) }}</li>
@endsection
@section('content')
@php $isEn = ($locale ?? 'en') === 'en'; @endphp
<div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-2">
    <div><h2 style="display:flex;align-items:center;gap:.5rem">Manifesto <span class="locale-badge {{ $locale }}">{{ strtoupper($locale) }}</span></h2><p>Hanya {{ $isEn ? 'EN' : 'ID' }} tampil</p></div>
...
<div class="card-white saas-card" style="max-width:640px">
    <form action="{{ route('admin.home.manifesto.update', ['locale'=>$locale]) }}" method="POST">
        @csrf @method('PUT')
        <div class="form-group">
            <label class="form-label">Subtitle — {{ $isEn ? 'EN' : 'ID' }} <span class="required">*</span></label>
            <input type="text" name="{{ $isEn ? 'subtitle_en' : 'subtitle_id' }}" class="form-control @error($isEn ? 'subtitle_en' : 'subtitle_id') is-invalid @enderror" value="{{ old($isEn ? 'subtitle_en' : 'subtitle_id', $isEn ? $settings['subtitle_en'] : $settings['subtitle_id']) }}" maxlength="60" data-max="60" required>
        ...
        <div class="form-group">
            ...
            <textarea name="{{ $isEn ? 'title_en' : 'title_id' }}" class="form-control @error($isEn ? 'title_en' : 'title_id') is-invalid @enderror" rows="4" maxlength="360" data-max="360" required>{{ old($isEn ? 'title_en' : 'title_id', $isEn ? $settings['title_en'] : $settings['title_id']) }}</textarea>
            @error($isEn ? 'title_en' : 'title_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
            <div style="font-size:.68rem;color:var(--gray-400);margin-top:.25rem">Use *word* to mark bold</div>
        </div>
