...
@section('title', 'Add Engagement Model')
@section('page-title', 'Add Engagement Model')
@section('breadcrumb')
...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h2>Add New Engagement Model</h2>
        <p>Create a new engagement model for the services page</p>
    </div>
...
<div x-data="engForm()" style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;">
    ...
    <div class="card-white">
        ...
        <form action="{{ route('admin.services.engagement.store') }}" method="POST" id="engForm">
            @csrf
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;">
                <div>
                    <label class="form-label">Letter <span style="color:var(--green-500)">*</span></label>
                    <input type="text" name="letter" class="form-control @error('letter') is-invalid @enderror" value="{{ old('letter') }}" maxlength="10" required x-model="letter" @input="letter = letter.toUpperCase()">
                    @error('letter') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    <div class="form-text">e.g. "A", "B", "C"</div>
                </div>
                <div>
                    <label class="form-label">Sort Order</label>
                    <input type="number" name="sort_order" class="form-control" value="{{ old('sort_order', 0) }}" x-model.number="sortOrder" min="0">
                </div>
            </div>
            <div style="margin-top:.75rem;">
                <label class="form-label">Title <span style="color:var(--green-500)">*</span></label>
                <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title') }}" required x-model="title">
                @error('title') <div class="invalid-feedback">{{ $message }}</div> @enderror
                <div class="form-text">e.g. "One-Off Projects"</div>
            </div>
            <div style="margin-top:.75rem;">
            ...
            <div style="margin-top:.75rem;">
                <div style="display:flex;align-items:center;gap:.5rem;">
                    ...
                    <input type="checkbox" name="is_active" value="1" class="form-check-input" style="width:20px;height:20px;" {{ old('is_active', 1) ? 'checked' : '' }} x-model="isActive">
                    <span style="font-size:.8125rem;color:var(--gray-600)" x-text="isActive ? 'Active' : 'Inactive'"></span>
                </div>
            </div>
            <div style="margin-top:1.25rem;">
                <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg"></i> Create Model</button>
            </div>
        </form>
    </div>
    ...
    <div>
        ...
        <div style="margin-top:1rem;padding:.75rem;background:var(--gray-50);border-radius:var(--radius-md);font-size:.75rem;color:var(--gray-500);">
            <i class="bi bi-info-circle"></i> Preview shows how the model will appear on the services page. Card uses the green-50 background for the letter circle.
        </div>
    </div>
</div>
@endsection
