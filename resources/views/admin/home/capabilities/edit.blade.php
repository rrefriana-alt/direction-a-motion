...
@section('title', 'Edit: ' . $capability->title)
...
<div style="display:grid;grid-template-columns:1fr 340px;gap:1.5rem;align-items:start">
    <div class="card-white" style="max-width:640px">
        <form action="{{ route('admin.home.capabilities.update', $capability->id) }}" method="POST" enctype="multipart/form-data" id="capForm">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label class="form-label">Title <span style="color:var(--red-600)">*</span></label>
                <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" id="titleInput" value="{{ old('title', $capability->title) }}" required>
                @error('title') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            <div class="form-group">
                <label class="form-label">Description <span style="color:var(--red-600)">*</span></label>
                <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="descInput" rows="3" required>{{ old('description', $capability->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                @php
                    ...
                    $tagsArray = [];
                    if ($capability->tags) {
                        $decoded = json_decode($capability->tags, true);
                        $tagsArray = is_array($decoded) ? $decoded : [];
                ...
                <input type="hidden" name="tags" id="tagsHidden" value="{{ old('tags', $tagsValue) }}">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="number" id="numberInput" value="{{ old('number', $capability->number) }}" min="0">
                    <div style="font-size:.72rem;color:var(--gray-400);margin-top:.25rem">Card number (01, 02, etc.)</div>
                </div>
                <div class="form-group">
                    <label class="form-label">Sort Order</label>
                    <input type="number" class="form-control" name="sort_order" value="{{ old('sort_order', $capability->sort_order) }}" min="0">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div id="currentImage" style="margin-bottom:.5rem">
                    <img src="{{ asset('img/' . $capability->image) }}" alt="Current image" style="max-width:100%;max-height:100px;border-radius:var(--radius-sm);border:1px solid var(--gray-200)">
                </div>
                @endif
                <div id="dropZone" style="border:2px dashed var(--gray-200);border-radius:var(--radius-md);padding:1.5rem;text-align:center;cursor:pointer;transition:all .2s" onclick="document.getElementById('imageInput').click()">
                    ...
                    <div style="font-size:.8rem;color:var(--gray-500);margin-top:.25rem">{{ $capability->image ? 'Click to replace image' : 'Click to upload or drag and drop' }}</div>
                ...
                <div id="imagePreview" style="margin-top:.5rem;display:none">
                    <img id="previewImg" style="max-width:100%;max-height:100px;border-radius:var(--radius-sm);border:1px solid var(--gray-200)">
            ...
            <div class="form-group">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" name="is_active" value="1" {{ old('is_active', $capability->is_active) ? 'checked' : '' }}>
    ...
    <div>
        ...
        <div class="card-white" style="padding:0;overflow:hidden">
            <div style="height:140px;background:var(--gray-900);overflow:hidden;display:flex;align-items:center;justify-content:center">
                @if($capability->image)
                <img id="cardPreviewImg" src="{{ asset('img/' . $capability->image) }}" style="width:100%;height:100%;object-fit:cover">
                @else
                <img id="cardPreviewImg" style="display:none;width:100%;height:100%;object-fit:cover">
                @endif
                <div id="cardPreviewPlaceholder" style="{{ $capability->image ? 'display:none' : '' }}color:var(--gray-600)"><i class="bi bi-image" style="font-size:1.5rem"></i></div>
            </div>
            <div style="padding:1rem">
                <div style="font-size:.65rem;font-weight:700;color:var(--green-600);letter-spacing:.05em;margin-bottom:.5rem" id="cardPreviewNumber">{{ str_pad($capability->number, 2, '0', STR_PAD_LEFT) }}</div>
                <div style="font-size:.9rem;font-weight:600;color:var(--gray-900);margin-bottom:.25rem" id="cardPreviewTitle">{{ $capability->title }}</div>
                <div style="font-size:.73rem;color:var(--gray-500);margin-bottom:.75rem" id="cardPreviewDesc">{{ $capability->description }}</div>
                <div class="d-flex gap-1" style="flex-wrap:wrap" id="cardPreviewTags">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let tags = {!! json_encode($tagsArray) !!};
