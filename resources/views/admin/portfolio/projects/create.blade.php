                ...
                <div class="group-card">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem">
                        ...
                        <div class="field-group">
                            <label class="field-label">Category <span style="color:var(--danger)">*</span> <a href="{{ route('admin.portfolio.projects.categories', ['locale'=>$locale ?? 'en']) }}" style="margin-left:.4rem;font-size:.6rem;font-weight:600;color:var(--green-600);text-transform:none;letter-spacing:0">Manage</a></label>
                            <div style="display:flex;gap:.4rem">
                                <select name="category" id="categorySelect" class="form-select" required style="flex:1"><option value="">Select category</option>@foreach($categories as $key=>$label)<option value="{{ $key }}" {{ old('category')===$key?'selected':'' }}>{{ $label }}</option>@endforeach</select>
...
function openCatModal(){document.getElementById('catModal').style.display='flex';document.getElementById('catNewLabel').value='';const e=document.getElementById('catModalErr');if(e) e.style.display='none';const m=document.getElementById('catInlineMsg');if(m) m.style.display='none';setTimeout(()=>document.getElementById('catNewLabel').focus(),50);}
...
async function submitCatModal(){
    ...
    try{
        const res=await fetch("{{ route('admin.portfolio.projects.categories.store', ['locale'=>$locale ?? 'en']) }}",{
