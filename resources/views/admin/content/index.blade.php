...
@section('page-title', 'Page Section Manager')
...
<div class="page-tabs">
    ...
    <a href="?page={{ $key }}" class="page-tab {{ $activePage===$key ? 'active' : '' }}">
        <i class="bi {{ $pg['icon'] }}"></i> {{ $pg['label'] }}
    </a>
    @endforeach
</div>
...
<div style="display:flex;align-items:center;gap:.85rem;margin-bottom:1.5rem;">
    <div style="width:42px;height:42px;border-radius:11px;background:{{ $pages[$activePage]['color'] }};color:{{ $pages[$activePage]['tcolor'] }};display:flex;align-items:center;justify-content:center;font-size:1.1rem;">
        <i class="bi {{ $pages[$activePage]['icon'] }}"></i>
    </div>
    <div>
        <h2 style="font-size:1.1rem;font-weight:800;color:var(--tx-primary);margin:0;">{{ $pages[$activePage]['label'] }} — Section Manager</h2>
...
<div class="section-card">
    <div class="section-thumb" style="background:{{ $sec['color'] }};color:{{ $sec['tcolor'] }};">
        <i class="bi {{ $sec['icon'] }}" style="font-size:1.6rem;"></i>
    </div>
    <div class="section-info">
        ...
        <div class="section-fields">
            ...
            <span class="section-field-tag">{{ $f }}</span>
            @endforeach
        </div>
    </div>
    <button class="btn btn-ghost btn-sm" onclick="openDrawer('home_{{ $sec['id'] }}')" style="flex-shrink:0;">
...
<div class="section-card">
    ...
    <div class="section-info">
        ...
        <div class="section-desc">{{ $sec['desc'] }}</div>
        <div class="section-fields">
            @foreach($sec['fields'] as $f)<span class="section-field-tag">{{ $f }}</span>@endforeach
        </div>
    </div>
    <button class="btn btn-ghost btn-sm" onclick="openDrawer('work_{{ $sec['id'] }}')" style="flex-shrink:0;">
        <i class="bi bi-pencil-fill" style="font-size:.72rem;"></i> Edit
    </button>
</div>
@endforeach
...
@foreach($servicesSections as $sec)
<div class="section-card">
    <div class="section-thumb" style="background:{{ $sec['color'] }};color:{{ $sec['tcolor'] }};">
        <i class="bi {{ $sec['icon'] }}" style="font-size:1.6rem;"></i>
    </div>
    ...
    <button class="btn btn-ghost btn-sm" onclick="openDrawer('services_{{ $sec['id'] }}')" style="flex-shrink:0;">
        <i class="bi bi-pencil-fill" style="font-size:.72rem;"></i> Edit
    </button>
</div>
@endforeach
...
<div class="section-card">
    ...
    <button class="btn btn-ghost btn-sm" onclick="openDrawer('about_{{ $sec['id'] }}')" style="flex-shrink:0;">
        <i class="bi bi-pencil-fill" style="font-size:.72rem;"></i> Edit
    </button>
</div>
@endforeach
...
<div class="section-card">
    ...
    <button class="btn btn-ghost btn-sm" onclick="openDrawer('contact_{{ $sec['id'] }}')" style="flex-shrink:0;">
        <i class="bi bi-pencil-fill" style="font-size:.72rem;"></i> Edit
    </button>
</div>
@endforeach
...
<div class="drawer" id="drawer-home_hero">
    ...
    <form action="{{ route('admin.content.update') }}" method="POST" class="d-flex flex-column h-full" style="flex:1;display:flex;flex-direction:column;">
        @csrf
        <div class="drawer-body">
            <div class="field-group">
                <div class="field-label">Tagline <span class="field-hint">Small text above the headline</span></div>
                <input type="text" name="home_hero_tagline" class="form-control" value="{{ old('home_hero_tagline', $settings['home_hero_tagline'] ?? 'Creative group · Est. 2016 · Bandung / Jakarta / Bali') }}">
            </div>
            <div class="field-group">
                <div class="field-label">Main Title <span class="field-hint">Large hero headline</span></div>
                <textarea name="home_hero_title" class="form-control" rows="3" placeholder="e.g. Create to Elevate">{{ old('home_hero_title', $settings['home_hero_title'] ?? "Create\nto\nElevate") }}</textarea>
            </div>
            <div class="field-group">
                <div class="field-label">Description <span class="field-hint">Short body text below the headline</span></div>
                <textarea name="home_hero_desc" class="form-control" rows="3">{{ old('home_hero_desc', $settings['home_hero_desc'] ?? 'Design · Production House · Events · Merch. An Indonesian creative group since 2016.') }}</textarea>
            </div>
        </div>
...
<div class="drawer" id="drawer-home_manifesto">
    ...
    <form action="{{ route('admin.content.update') }}" method="POST" style="flex:1;display:flex;flex-direction:column;">
        @csrf
        <div class="drawer-body">
            <div class="field-group">
                <div class="field-label">Subtitle <span class="field-hint">Label above the manifesto, e.g. MANIFESTO</span></div>
                <input type="text" name="home_s1_subtitle" class="form-control" value="{{ old('home_s1_subtitle', $settings['home_s1_subtitle'] ?? 'MANIFESTO') }}">
            </div>
            <div class="field-group">
                <div class="field-label">Statement <span class="field-hint">Use *asterisks* to highlight words in green</span></div>
                <textarea name="home_s1_title" class="form-control" rows="5">{{ old('home_s1_title', $settings['home_s1_title'] ?? 'Every brief can be solved with *creativity, an *innovative route, and execution that actually lands.') }}</textarea>
            </div>
        </div>
        <div class="drawer-footer">
            <button type="button" class="btn btn-ghost btn-sm" style="margin-left:auto;" onclick="closeDrawer()">Cancel</button>
    ...
    <form action="{{ route('admin.content.update') }}" method="POST" style="flex:1;display:flex;flex-direction:column;">
        ...
        <div class="drawer-body">
            <div class="field-group">
                ...
                <input type="text" name="home_s2_title" class="form-control" value="{{ old('home_s2_title', $settings['home_s2_title'] ?? 'We are not just thinkers. We are makers.') }}">
            </div>
        </div>
        <div class="drawer-footer">
            <button type="button" class="btn btn-ghost btn-sm" style="margin-left:auto;" onclick="closeDrawer()">Cancel</button>
...
<div class="drawer" id="drawer-home_work_ov">
    ...
    <form action="{{ route('admin.content.update') }}" method="POST" style="flex:1;display:flex;flex-direction:column;">
        ...
        <div class="drawer-body">
            <div class="field-group">
                <div class="field-label">Section Title</div>
                <input type="text" name="home_s3_title" class="form-control" value="{{ old('home_s3_title', $settings['home_s3_title'] ?? 'Selected Works') }}">
            </div>
        </div>
        <div class="drawer-footer">
            <button type="button" class="btn btn-ghost btn-sm" style="margin-left:auto;" onclick="closeDrawer()">Cancel</button>
...
<div class="drawer" id="drawer-home_process">
    ...
    <form action="{{ route('admin.content.update') }}" method="POST" style="flex:1;display:flex;flex-direction:column;">
        ...
        <div class="drawer-body">
            <div class="field-group">
                <div class="field-label">Section Title</div>
                <input type="text" name="home_s4_title" class="form-control" value="{{ old('home_s4_title', $settings['home_s4_title'] ?? 'Trusted by industry leaders') }}">
            </div>
        </div>
        <div class="drawer-footer">
            <button type="button" class="btn btn-ghost btn-sm" style="margin-left:auto;" onclick="closeDrawer()">Cancel</button>
...
<div class="drawer" id="drawer-home_news_ov">
    ...
    <form action="{{ route('admin.content.update') }}" method="POST" style="flex:1;display:flex;flex-direction:column;">
        ...
        <div class="drawer-body">
            <div class="field-group">
                <div class="field-label">Section Title</div>
                <input type="text" name="home_s5_title" class="form-control" value="{{ old('home_s5_title', $settings['home_s5_title'] ?? 'News & Insights') }}">
            </div>
        </div>
        <div class="drawer-footer">
            <button type="button" class="btn btn-ghost btn-sm" style="margin-left:auto;" onclick="closeDrawer()">Cancel</button>
    ...
    <form action="{{ route('admin.content.update') }}" method="POST" style="flex:1;display:flex;flex-direction:column;">
        ...
        <div class="drawer-body">
            <div class="field-group">
                ...
                <input type="text" name="home_s6_title" class="form-control" value="{{ old('home_s6_title', $settings['home_s6_title'] ?? 'Ready to elevate your brand?') }}">
            </div>
            <div class="field-group">
                <div class="field-label">Quote <span class="field-hint">Italic quote below the headline</span></div>
                <textarea name="home_s6_quote" class="form-control" rows="3">{{ old('home_s6_quote', $settings['home_s6_quote'] ?? 'The best work happens when brave clients meet a relentless creative team.') }}</textarea>
            </div>
        </div>
        <div class="drawer-footer">
            <button type="button" class="btn btn-ghost btn-sm" style="margin-left:auto;" onclick="closeDrawer()">Cancel</button>
    ...
    <form action="{{ route('admin.content.update') }}" method="POST" style="flex:1;display:flex;flex-direction:column;">
        ...
        <div class="drawer-body">
            <div class="field-group">
                ...
                <input type="text" name="about_founder_name" class="form-control" value="{{ old('about_founder_name', $settings['about_founder_name'] ?? 'Sona Lesmana') }}">
            </div>
            <div class="field-group">
                <div class="field-label">Title / Role</div>
                <input type="text" name="about_founder_title" class="form-control" value="{{ old('about_founder_title', $settings['about_founder_title'] ?? 'Founder & CEO') }}">
            </div>
            <div class="field-group">
                <div class="field-label">Signature Quote</div>
                <textarea name="about_founder_quote" class="form-control" rows="3">{{ old('about_founder_quote', $settings['about_founder_quote'] ?? 'Creativity without execution is just a hallucination.') }}</textarea>
            </div>
        </div>
        <div class="drawer-footer">
            <button type="button" class="btn btn-ghost btn-sm" style="margin-left:auto;" onclick="closeDrawer()">Cancel</button>
    ...
    <form action="{{ route('admin.content.update') }}" method="POST" style="flex:1;display:flex;flex-direction:column;">
        ...
        <div class="drawer-body">
            <div class="field-group">
                ...
                <input type="email" name="contact_email" class="form-control" value="{{ old('contact_email', $settings['contact_email'] ?? 'hello@fugocreativegroup.com') }}">
            </div>
            <div class="field-group">
                <div class="field-label">Phone Number</div>
                <input type="text" name="contact_phone" class="form-control" value="{{ old('contact_phone', $settings['contact_phone'] ?? '+62 821 2100 0680') }}">
            </div>
            <div class="field-group">
                <div class="field-label">Office Address (Bandung)</div>
                <textarea name="contact_address_bdg" class="form-control" rows="2">{{ old('contact_address_bdg', $settings['contact_address_bdg'] ?? 'Jl. Permata Taman Sari Raya No.21, Bandung') }}</textarea>
            </div>
            <div class="field-group">
                <div class="field-label">Instagram URL</div>
                <input type="text" name="contact_instagram" class="form-control" value="{{ old('contact_instagram', $settings['contact_instagram'] ?? '') }}">
            </div>
            <div class="field-group">
                <div class="field-label">LinkedIn URL</div>
                <input type="text" name="contact_linkedin" class="form-control" value="{{ old('contact_linkedin', $settings['contact_linkedin'] ?? '') }}">
            </div>
        </div>
        <div class="drawer-footer">
            <button type="button" class="btn btn-ghost btn-sm" style="margin-left:auto;" onclick="closeDrawer()">Cancel</button>
