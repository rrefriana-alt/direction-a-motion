...
<link rel="canonical" href="{{ route('journal.show', $post->slug) }}">
...
<meta property="og:url" content="{{ route('journal.show', $post->slug) }}">
...
<section class="phead">
  ...
  <div class="shell">
    <div class="article">
      <p class="crumb fade-up"><a href="{{ url('') }}">Fugo</a> <span>/</span> <a href="{{ route('journal.index') }}" data-en="Journal" data-id="Jurnal">Journal</a> <span>/</span> <span>{{ $post->category_display }}</span></p>
      ...
      <div class="abyline abyline--bare fade-up" data-delay="2">
        ...
        <div>
          ...
          <span>{{ $post->published_date?->format('d M Y') }} &nbsp;·&nbsp; {{ $post->view_count }} <span data-en="reads" data-id="dibaca">reads</span></span>
        </div>
        <div class="ashare">
          <button class="ashare__btn" type="button" data-copylink="{{ route('journal.show', $post->slug) }}" aria-label="Copy link">
            ...
            <span class="ashare__tip" data-en="Copy link" data-id="Salin tautan">Copy link</span>
          </button>
          <a class="ashare__btn" href="https://wa.me/?text={{ urlencode($post->title . ' ' . route('journal.show', $post->slug)) }}" target="_blank" rel="noopener" aria-label="Share on WhatsApp">
          ...
          <a class="ashare__btn" href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(route('journal.show', $post->slug)) }}" target="_blank" rel="noopener" aria-label="Share on LinkedIn">
  ...
  <div class="shell">
    <div class="article">
      ...
      <div class="card__tags fade-up" style="margin-top:2.2rem">
        <a class="tag" href="{{ route('journal.index', ['category' => $post->category]) }}" style="text-decoration:none">{{ $post->category_display }}</a>
        <a class="tag" href="{{ route('journal.index') }}" style="text-decoration:none" data-en="Studio journal" data-id="Jurnal studio">Studio journal</a>
      </div>
      <div class="abyline fade-up">
        <span class="bfeat__avatar" aria-hidden="true">{{ strtoupper(mb_substr($post->author ?? 'F', 0, 1)) }}</span>
        <div>
          <strong>{{ $post->author }}</strong>
          <span data-en="Contributor, Fugo Creative Group — Bandung / Jakarta / Bali" data-id="Kontributor, Fugo Creative Group — Bandung / Jakarta / Bali">Contributor, Fugo Creative Group — Bandung / Jakarta / Bali</span>
        </div>
      </div>
      ...
      <nav class="aprevnext fade-up" aria-label="More articles">
        ...
        <a class="aprevnext__link" href="{{ route('journal.show', $newerPost->slug) }}">
          <span class="bcard__meta"><span aria-hidden="true">←</span> <span data-en="Newer" data-id="Baru">Newer</span></span>
        ...
        <a class="aprevnext__link aprevnext__link--next" href="{{ route('journal.show', $olderPost->slug) }}">
          <span class="bcard__meta"><span data-en="Older" data-id="Lama">Older</span> <span aria-hidden="true">→</span></span>
      ...
      <div class="mt-l fade-up">
        <a class="tlink green" href="{{ route('journal.index') }}" data-en="← Back to journal" data-id="← Kembali ke jurnal">← Back to journal</a>
      </div>
    </div>
  </div>
</section>
...
<section class="section" style="padding-top:0">
  <div class="shell">
    <div class="row between end gap-m">
      <div>
        <p class="eyebrow"><span data-en="Keep reading" data-id="Lanjut baca">Keep reading</span></p>
