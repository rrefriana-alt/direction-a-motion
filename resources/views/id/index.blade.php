...
<section class="hero">
  ...
  <div class="shell hero__in">
    ...
    <div class="hero__meta">
      <p class="lede" data-delay="1">{{ $heroDescription }}</p>
    ...
    <div class="row between center mt-m" data-delay="3">
      ...
      <p class="mono faint">{{ $heroTagline }}</p>
    </div>
  </div>
</section>
  ...
  <div class="shell">
    <div class="stats">
      ...
      <div class="stat fade-up"{{ $index > 0 ? ' data-delay="' . $index . '"' : '' }}>
        ...
        <p class="stat__l">{{ $stat->labelLocalized('id') }}</p>
      </div>
      @endforeach
    </div>
  </div>
  ...
  <div class="shell">
    <div class="svc__grid">
      ...
      <div class="svc__sticky">
        ...
        @php
            $capTitle = \App\Models\Setting::localized('home_capabilities_title', 'id', "Five studios,<br>one standard");
            $capDesc = \App\Models\Setting::localized('home_capabilities_description', 'id', 'Brief one team and get the whole chain — strategy, design, film, stage and physical product — without the agency handoff tax.');
        @endphp
        <h2 class="display h-xxl mt-s fade-up" data-delay="1">{!! \App\Support\Works::tint($capTitle) !!}</h2>
        <p class="lede mt-s fade-up" data-delay="2">
          {{ $capDesc }}</p>

        <ul class="svc__index" aria-hidden="true">
          @foreach($capabilities as $index => $cap)
          <li class="svc__idx {{ $index === 0 ? 'on' : '' }}"><span class="n">{{ str_pad($cap->number, 2, '0', STR_PAD_LEFT) }}</span><span class="t">{{ $cap->titleLocalized('id') }}</span></li>
          @endforeach
        </ul>
      ...
      <div class="svc__panels">
          ...
          <article class="card fade-up" data-cursor="{{ $cap->titleLocalized('id') }}">
            <div class="card__art">
              @if($cap->image)
            ...
            <h3>{{ $cap->titleLocalized('id') }}</h3>
            <p>{{ $cap->descLocalized('id') }}</p>
...
<section class="section" style="padding-bottom:0">
  <div class="shell row between end gap-m">
    <div>
      ...
      <h2 class="display h-xxl mt-s fade-up" data-delay="1">Bukti, bukan<br>janji</h2>
    </div>
...
<section class="section">
  <div class="shell">
    ...
    <h2 class="display h-xl mt-s mb fade-up" data-delay="1" style="margin-bottom:2.5rem">Brand teregulasi, menuntut,<br>dan sangat terlihat</h2>

    <div class="sectors fade-up" data-delay="2">
      @foreach($sectors as $sector)
      <div class="sector">
        <h4>{{ !empty($sector->heading_id) ? $sector->heading_id : $sector->heading_en }}</h4>
        <ul>
...
<section class="section" style="padding-top:0">
  <div class="shell grid g-12">
    <div class="col-4">
      <p class="eyebrow">{!! \App\Support\Works::tint($processEyebrow) !!}</p>
      <h2 class="display h-xl mt-s fade-up" data-delay="1">{!! \App\Support\Works::tint($processTitleEn) !!}</h2>
    </div>
    <div class="col-8">
      <div class="steps">
        ...
        <div class="step fade-up"{{ $index > 0 ? ' data-delay="' . $index . '"' : '' }}>
          ...
          <div class="step__b">
            <h3>{{ !empty($step->title_id) ? $step->title_id : $step->title_en }}</h3>
            <p>{{ !empty($step->description_id) ? $step->description_id : $step->description_en }}</p>
          </div>
        </div>
        @endforeach
      </div>
      ...
      <div>
        ...
        <div class="quote__by">
          ...
          <span><strong style="color:var(--ink)">{{ $founderName }}</strong><br>
            <span>{{ $founderTitle }}, Fugo Creative Group</span></span>
        </div>
      </div>
...
<section class="section" style="padding-top:0">
  <div class="shell">
    <div class="row between end gap-m">
      <div>
        <p class="eyebrow">{!! $journalEyebrow ?? '07 — Jurnal' !!}</p>
        <h2 class="display h-xxl mt-s fade-up" data-delay="1">{!! $journalTitle ?? 'Catatan dari<br>studio' !!}</h2>
        <p class="lede mt-s fade-up" data-delay="2">{{ $journalLede ?? 'Catatan proses, cerita proyek, dan pandangan soal industri — ditulis oleh orang-orang yang membuat karyanya.' }}</p>
      </div>
      <a class="tlink fade-up" data-delay="2" href="{{ route('journal.index', ['locale'=>$locale]) }}">{{ $journalCtaLabel ?? 'Semua artikel →' }}</a>
    </div>
      ...
      <div class="bfeat__side">
        ...
        <article class="bmini fade-up" data-delay="{{ $j + 1 }}" data-cursor="Read">
          ...
          <div class="bmini__txt">
            ...
            <a class="tlink green" href="{{ route('journal.show', ['locale'=>$locale, 'slug'=>$post->slug]) }}">Baca →</a>
          </div>
        </article>
        @endforeach
      ...
      <div class="bfeat__side">
        ...
        <article class="bmini fade-up" data-delay="{{ $j + 1 }}">
          ...
          <div class="bmini__txt">
            ...
            <a class="tlink green" href="{{ route('journal.index', ['locale'=>$locale]) }}">Lihat →</a>
          </div>
        </article>
        @endforeach
        <a class="bmini bmini--more fade-up" data-delay="3" href="{{ route('journal.index', ['locale'=>$locale]) }}">
...
<section class="section cta">
  ...
  <div class="shell">
    <p class="eyebrow is-plain fade-up" style="justify-content:center">{{ $ctaEyebrow }}</p>
    <h2 class="display cta__big mt-s fade-up" data-delay="1">{!! \App\Support\Works::tint($ctaTitle) !!}</h2>
