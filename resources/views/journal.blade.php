...
<section class="phead">
  ...
  <div class="shell">
    <p class="crumb fade-up"><a href="{{ url('') }}">Fugo</a> <span>/</span> <span data-en="Journal" data-id="Jurnal">Journal</span></p>
    <h1 class="display h-xxl mt-s fade-up" data-delay="1" data-en="Studio&lt;br&gt;journal" data-id="Jurnal&lt;br&gt;studio">Studio<br>journal</h1>
    <p class="lede mt-m fade-up" data-delay="2" data-en="Process notes, project stories and takes on the industry — written by the people who make the work." data-id="Catatan proses, cerita proyek, dan pandangan soal industri — ditulis oleh orang-orang yang membuat karyanya.">Process notes, project stories and takes on the industry — written by the people who make the work.</p>
  </div>
</section>
...
<section class="section" style="padding-top:clamp(2rem,4vw,3rem)">
  <div class="shell">
    <div class="chips fade-up" style="margin-bottom:2.5rem" role="group" aria-label="Filter">
      <a class="chip {{ empty($activeCategory) ? 'on' : '' }}" href="{{ route('journal.index') }}" data-en="All" data-id="Semua">All</a>
    ...
    @if($posts->count())
    <p class="mono faint fade-up" style="margin-bottom:1.8rem">{{ $posts->total() }} <span data-en="articles" data-id="artikel">articles</span></p>
    <div class="bgrid">
      ...
      <article class="bcard fade-up" data-delay="{{ $i % 3 }}" data-cursor="Read">
        ...
        <div class="bcard__body">
          ...
          <h3><a href="{{ route('journal.show', $post->slug) }}">{{ $post->title }}</a></h3>
    ...
    @endif
    @else
    <p class="lede muted" data-en="No articles yet — check back soon." data-id="Belum ada artikel — silakan kembali lagi.">No articles yet — check back soon.</p>
    @endif
  </div>
</section>
