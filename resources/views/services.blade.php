...
<section class="phead">
  ...
  <div class="shell">
    ...
    <h1 class="display h-xxl mt-s fade-up" data-delay="1">{!! \App\Models\Setting::get('services_page_headline', 'Five studios,<br>one invoice') !!}</h1>
    <p class="lede mt-m fade-up" data-delay="2">{{ \App\Models\Setting::get('services_page_subtitle', 'Most agencies subcontract at least half of this. We do not — which is why the schedule holds and the brand stays consistent across every touchpoint.') }}</p>
  </div>
</section>
<section class="section">
  <div class="shell stack gap-xl">
    @foreach($categories as $index => $category)
    <article class="card fade-up" id="s{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}" data-cursor="{{ $category->name }}">
            ...
            <div style="padding-block:1rem;border-top:1px solid var(--line)">
              ...
              <dd class="muted" style="font-size:.92rem;margin-top:.35rem">
                @if($detail->items && $detail->items->count())
                  {{ \Illuminate\Support\Str::replace('>', '', $detail->items->pluck('item_name')->implode(', ')) }}
                @else
                  {{ \Illuminate\Support\Str::replace('>', '', $detail->content) }}
@endif
              </dd>
            </div>
            @endforeach
  ...
  <div class="shell">
    ...
    <div class="grid g-12 mt-l">
      ...
      <div class="col-4 card fade-up" @if($index > 0) data-delay="{{ $index }}" @endif>
        <p class="card__num">{{ $engagement->letter }}</p><h3>{{ $engagement->title }}</h3>
